...
bool AAbility_PossessCreeps::Ability()
{
	...
	bIsCasting = true; 
	//Spawn Widget if it's not already created 
	if(CastTimerWidgetClass != nullptr && castBarWidget == nullptr)
	{
		if (GetOwner()->IsA(AHeroBase::StaticClass()) && !GetOwner()->ActorHasTag("AI"))
		{
			AHeroBase* owningHero = Cast<AHeroBase>(GetOwner());
			if (owningHero)
			{
				APlayerController* playerController = Cast<APlayerController>(owningHero->GetController());
				if (playerController)
				{
