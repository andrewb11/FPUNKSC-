...
void AHeroBase::BeginPlay()
{
	...
	respawnEffect = GetWorld()->SpawnActor<ARespawnOverTime>(respawnClass, FVector::ZeroVector, FRotator::ZeroRotator, spawnParams);
...
void AHeroBase::OnOverlapBegin(class UPrimitiveComponent* ThisComp, class AActor* OtherActor, class UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult &SweepResult)
{
	if (Cast<ACreepCamp>(OtherActor))
	{
		...
		if (!Tags.Contains("AI"))
		{
			if (campProgressWidget)
			{
				campProgressWidget->SetCurrentCreepCamp(visitingCamp);
				campProgressWidget->SetVisibility(ESlateVisibility::Visible);
			}
		}
		SetIsCapturing(true, visitingCamp);
	}
}
...
void AHeroBase::RecruitCreep()
{
	if (visitingCamp)
	{
		if (currentArmySize < maxArmySize && visitingCamp->GetCreepArray().Num() > 0)
		{
...
void AHeroBase::RestoreFriction()
{
	if (restoreFrictionTimerHandle.IsValid())
	{
		GetWorld()->GetTimerManager().ClearTimer(restoreFrictionTimerHandle);
	}
...
void AHeroBase::Launch()
{
	...
	LaunchCharacter(GetActorForwardVector() *= launchForce, true, false);
	GetWorld()->GetTimerManager().SetTimer(restoreFrictionTimerHandle, this, &AHeroBase::RestoreFriction, 0.2f, true);
}

void AHeroBase::ChainCombo()
{
